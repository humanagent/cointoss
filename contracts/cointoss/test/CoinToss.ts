...
describe("CoinToss", function () {
  ...
  describe("Deployment", function () {
    it("Should set the correct tossing token and max tossing amount", async function () {
      const { coinToss, usdcAddress, maxTossingAmountPerOutcome } =
        await loadFixture(deployCoinToss);
      expect((await coinToss.tossingTokenAddress()).toLowerCase()).to.equal(
        usdcAddress.toLowerCase(),
      );
      expect(await coinToss.maxTossingAmountPerOutcome()).to.equal(
        maxTossingAmountPerOutcome,
      );
    });
  ...
  describe("Create Toss", function () {
    it("Should create a toss", async function () {
      const { coinToss, owner, maxTossingAmountPerOutcome } =
        await loadFixture(deployCoinToss);
      const condition = "Will it rain tomorrow?";
      const outcomes = ["Yes", "No"];
      const tossingAmounts = [
        maxTossingAmountPerOutcome,
        maxTossingAmountPerOutcome,
      ];
      ...
      const [tossInfo, tossOutcomes, tossTossingAmounts] =
        await coinToss.tossInfo(1);
  ...
  describe("Place Toss", function () {
    it("Should allow a player to place a toss", async function () {
      const { coinToss, owner, usdc, maxTossingAmountPerOutcome, baseHolder1 } =
        await loadFixture(deployCoinToss);
      ...
      await usdc
        .connect(signer1)
        .approve(coinToss.target, ethers.parseUnits("10", 6));
      // Place a toss
    ...
    it("Should make a toss and distribute winnings", async function () {
      ...
      expect(signer1BalanceAfter).to.equal(
        signer1BalanceBefore + expectedWinnings,
      );
    });
    ...
    it("Should revert if try to place a toss twice", async function () {
      ...
      await usdc
        .connect(signer1)
        .approve(coinToss.target, ethers.parseUnits("20", 6));
      await coinToss.connect(signer1).placeToss(1, 0);

      // Try to place second toss
      await expect(
        coinToss.connect(signer1).placeToss(1, 1),
    ...
    it("10 players make a toss (mixed outcomes) and resolve with distribute winnings", async function () {
      ...
      const signers = await Promise.all(
        [
          ...
          baseHolder9,
          baseHolder10,
        ].map((holder) => ethers.getImpersonatedSigner(holder)),
      );

      const balancesBefore = await Promise.all(
        signers.map((signer) => usdc.balanceOf(signer.address)),
      );
    ...
    it("10 players all toss on the winning option", async function () {
      const {
        coinToss,
        owner,
        usdc,
      ...
      // Place tosses - all players toss on outcome 0
      for (let i = 0; i < signers.length; i++) {
        await usdc
          .connect(signers[i])
          .approve(coinToss.target, ethers.parseUnits("10", 6));
      ...
      for (let i = 0; i < signers.length; i++) {
        const balanceAfter = await usdc.balanceOf(signers[i].address);
        // Since all players toss on the winning outcome, their balances should remain unchanged
    ...
    it("2 players make a 0$ toss and resolve", async function () {
      const { coinToss, owner, usdc, baseHolder1, baseHolder2 } =
        await loadFixture(deployCoinToss);
      ...
      expect(signer1BalanceAfter).to.equal(
        await usdc.balanceOf(signer1.address),
      );
      expect(signer2BalanceAfter).to.equal(
        await usdc.balanceOf(signer2.address),
      );
    ...
    it("Should revert when trying to resolve a toss before end time", async function () {
      ...
      await expect(
        coinToss.connect(owner).resolveToss(1, 0, true),
  ...
  describe("Admin and User Pause/Return Actions", function () {
    ...
    it("Admin pauses toss and users claim funds", async function () {
      ...
      await expect(
        coinToss.connect(signer1).claimTokensFromPausedToss(1),
      ).to.be.revertedWithCustomError(coinToss, "PlayerDidNotToss");
      await expect(
        coinToss.connect(signer2).claimTokensFromPausedToss(1),
      ).to.be.revertedWithCustomError(coinToss, "PlayerDidNotToss");
    });
  });
});
